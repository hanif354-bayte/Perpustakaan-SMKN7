...
export function* getSearchByBorrowDateAdminSaga({ payload: { date } }) {
    try {
        const response = yield call(
            axios.get,
            URL_SEARCH_BORROW_BY_DATE_ADMIN.replace(':startDate', date.startDate).replace(':endDate', date.endDate)
        );

        yield put(getSearchByBorrowDateAdminSuccess(response.data));
    } catch (err) {
        yield put(getSearchByBorrowDateAdminFailed(err.response?.data || "Error fetching data"));
    }
}
