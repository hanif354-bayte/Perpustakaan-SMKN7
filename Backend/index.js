...
async function handleMessages(data, denda) {
    ...
    const tasks = data.map(async (item) => {
        const { id, judul, wa: number, name, updated_at, due_date, pengembalian } = item;
        if (pengembalian === 'selesai' || !number) return;

        const terlambat = await calculateLateDays(due_date, moment());
        const pesan = generateMessage(name, judul, updated_at, due_date, terlambat, denda, pengembalian);

        if (!pesan) return;

        await sendMessage(number, pesan, id);
    });
...
function generateMessage(name, judul, updated_at, due_date, terlambat, denda, pengembalian) {
    const formattedUpdatedAt = moment(updated_at).format('DD-MM-YYYY');
    const formattedDueDate = moment(due_date).format('DD-MM-YYYY');

    if (pengembalian === 'sukses') {
        return `Halo ${name}, buku \"${judul}\" yang kamu pinjam pada ${formattedUpdatedAt} sudah dikembalikan ${
            terlambat > 0 ? `dengan denda Rp${formatRupiah(terlambat * denda.nominal)}.` : 'dengan tepat waktu.'
        } Terima kasih.`;
    }

    return terlambat <= 0
        ? `Halo ${name}, buku \"${judul}\" yang kamu pinjam pada ${formattedUpdatedAt} harus dikembalikan pada ${formattedDueDate}.`
        : `Maaf ${name}, buku \"${judul}\" melewati batas waktu. Denda saat ini: Rp${formatRupiah(terlambat * denda.nominal)}.`;
}

// Helper function to send WhatsApp messages
async function sendMessage(number, pesan, id) {
    ...
    await whatsappService.updatePengembalian(id);
