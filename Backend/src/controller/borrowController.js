...
class borrowController {
    ...
    static async searchPeminjamanAdmin(req, res) {
        try {
            ...
            const borrows = await borrowService.searchPeminjamanAdmin(startDate, endDate);
            if (!borrows || borrows.length === 0) throw new Error("Data Tidak ditemukan");
    
            borrows.forEach((borrow) => {
                borrow.updated_at = moment(borrow.updated_at).format();
                borrow.due_date = borrow.due_date ? moment(borrow.due_date).format() : "-";
            });
